...
io.sockets.on('connection', function (socket) {
    // send initial state of board upon connection
    sendChessBoardState(socket);
    socket.on('chess board clicked', onChessBoardClicked);
});

function onChessBoardClicked (data) {
    var s = data.position;
    console.log("Got chess board clicked at position: " + s);
    i = (board[s] ^ y) & z;
    if (i > 8) {
    ...
    B=b;
    sendChessBoardState(this);
}

function sendChessBoardState (socket) {
    socket.emit('chess board state', {board: board, B: B, b: b});
}
